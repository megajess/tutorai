...
## Acceptance Criteria
- [ ] User can type a query into an input field and submit it
- [ ] Submitted query appears in the message history as a user message
- [ ] API response appears in the message history as an assistant message
- [ ] A loading indicator is shown while the API request is in flight
- [ ] Error state is shown if the API returns an error or is unreachable
- [ ] Message history scrolls correctly as new messages are added
- [ ] Input field is cleared after submission
- [ ] The UI is usable on a standard desktop browser width
